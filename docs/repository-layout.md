...
- `jobs/<job_id>/work/<content_id>/handoff-package/document-import/article-import.docx` 是从最终 Content
  派生的瞬时微信导入载体，可以按同一 Content 重新生成；它不是第七种业务产物，不进入源码仓库，
  也不能替代 Draft Receipt。
- `jobs/<job_id>/work/wechat-handoff.json` 是微信交接检查点，属于现有 Job 的恢复元数据，不是新业务产物。尚未闭环时不得清理；重启/换 Agent 必须从标准 prepare 恢复它。

...
> **阅读时机**：需要理解内容项目、载体选择、审计或批次交接时阅读。执行流程以 [`video-to-content` Skill](../.agents/skills/video-to-content/SKILL.md) 为准。
...
2. **正文与来源说明分层**：单人内容的正文默认采用作者直接口吻。原封面、UP 主、原标题、
   ...
   “这个视频认为”的外部总结者。
3. **文章配图默认来自原视频**：使用原视频封面和与正文论点、人物、实物、图表或演示
   对应的可追溯截帧，并在素材清单或元数据中记录时间点或时间范围。文章请求本身不授权
   自制结构图、AI 配图、图库图片或其他合成视觉；只有用户明确要求或单独批准才可使用。
   原视频没有合适画面时宁可少图，不能为了版面完整制造填充图。一图流等本身需要重新设计
   的载体，以用户明确选择该载体为授权，但不能反向扩展到另一篇文章的配图。
4. **本地图片是人工交接点**：不把相对路径、`file:`、`data:` 或 `blob:` 图片伪装成
   可复制图片。默认只在正文对应位置显示一行相对素材路径，上下留白由同一元素的 margin
   提供；机器校验信息放在 `data-local-image-slot` 等属性中。图片与简短清单随包交付，插图
   后最多只删除这一行路径。
5. **两层验证分别通过**：排版器的确定性检查只证明 HTML 能粘贴；本项目的 fidelity
...
├─ cover.jpg                     原视频封面
├─ assets/02-frame-00m12s.jpg    带来源时间点的视频截帧

...
来源声明集中在 metadata、署名区或一次必要披露中。正文已经转换成文章后，应直接承载来源表达，
而不是反复介绍“视频说了什么”；已经决定省略的非实质片段，也不应以标题、括号或说明继续占位。
这些都属于载体适配检查，不改变来源事实。
...
这不是全文去 AI 味，也不是固定公众号文风。审校后文章仍应保留来源的表达个性，只是 Agent
自己补写的载体语言更直接、更少模板感。
...
为了公众号载体凭空增加营销标题、整齐编号、固定结尾、导师口吻或“核心结论”模板。还包括载体
已经变成文章，却在正文反复用“视频认为”“视频指出”“创作者介绍”充当第三方报道壳；以及广告、
商品推广或平台 CTA 已被省略，却仍留下“视频原文中的推广段”“广告已删除”等占位标题或说明。

处理原则是恢复来源的直接叙述，并把非实质片段干净移除。不能把“视频”设为禁词：当视频本身是
实际讨论对象、发布行为需要说明或观点归属必须明确时，应保留。载体只能改善阅读，不能生成一个
新作者，也不能把内部编辑动作展示给读者。
...
3. 标题和总结是否没有增强来源主张；
4. 正文是否直接采用来源叙述视角，且没有反复的二手视频解说壳或省略片段占位；
5. 章节顺序、`material_sections`、图片 block 和 `visual_plan` 是否仍一致；
6. 所有 `expression_audit.items[*].after` 是否等于最终文档文本；
7. `content_validate.valid=true` 后才允许进入微信交接。

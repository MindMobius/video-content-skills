...
from ..core.srt import parse_srt, reconcile_cross_scale_cues, write_srt
from ..core.util import tail_text
...
def resolve_ocr_backend(
    ...
    if executable is None:
        raise OcrUnavailable(
            "VideOCR CLI is not installed or configured. Set VIDEO_SUBTITLE_VIDEOCR "
...
def ocr_doctor(options: VideOcrOptions) -> dict[str, Any]:
    ...
    if executable is None:
        return {
            ...
            "executable": None,
            "help": (
                "Install a VideOCR CLI release and set VIDEO_SUBTITLE_VIDEOCR "
                "to videocr-cli.exe."
            ),
        }
    return {
...
def _resolve_videocr_executable(value: str | None) -> Path | None:
    selected = (
        value
        or os.getenv("VIDEO_SUBTITLE_VIDEOCR")
        or os.getenv("SUBTITLE_AGENT_VIDEOCR")
        or os.getenv("VIDEOCR_BIN")
        or ""
    ).strip()
    if selected:

...
def read_requirements() -> dict[str, Any]:
    resource = files("video_content").joinpath("requirements.json")
    payload = json.loads(resource.read_text(encoding="utf-8"))
    if payload.get("schema_version") != "video-content/requirements-v1":
        raise ValueError("Unsupported video content requirements schema")
    return payload
...
def _next_step(status: str, *, deep: bool) -> str:
    if status == "ready":
        ...
        return "Run video_content_doctor with deep=true, then start the task."

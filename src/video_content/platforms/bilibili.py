...
class OpenCliSettings:
    ...
    def discover(
        ...
        *,
        ...
        opencli_value = (
            opencli
            or os.getenv("VIDEO_CONTENT_OPENCLI_COMMAND")
            or os.getenv("VIDEO_CONTENT_OPENCLI")
            or ""
        ).strip()
        ...
        selected_profile = profile or os.getenv("VIDEO_CONTENT_OPENCLI_PROFILE") or None
        selected_ytdlp = ytdlp or os.getenv("VIDEO_CONTENT_YTDLP") or None
        selected_ffmpeg = ffmpeg or os.getenv("VIDEO_CONTENT_FFMPEG") or None
        selected_browser_timeout = _positive_int_setting(
            browser_command_timeout_seconds,
            os.getenv("VIDEO_CONTENT_OPENCLI_BROWSER_TIMEOUT"),
            default=180,
            label="OpenCLI browser command timeout",
        )
        selected_download_retries = _nonnegative_int_setting(
            download_retries,
            os.getenv("VIDEO_CONTENT_DOWNLOAD_RETRIES"),
            default=2,
            label="download retries",
        )
        selected_retry_backoff = _nonnegative_float_setting(
            download_retry_backoff_seconds,
            os.getenv("VIDEO_CONTENT_DOWNLOAD_RETRY_BACKOFF"),
            default=2.0,
    ...
    if value:
        if value.startswith("["):
            try:
                ...
                raise ValueError(
                    "VIDEO_CONTENT_OPENCLI_COMMAND must be a JSON string array"
                ) from error
            if (
...
class OpenCliClient:
    ...
    def watch_later_adapter_available(self) -> bool:
        ...
        command += ["video-content-watch-later", "--help"]
        try:
    ...
    def watch_later(self, *, limit: int | None = None) -> list[dict[str, Any]]:
        args = ["video-content-watch-later", "list"]
        ...
        *,
        ...
        try:
            ...
            raise OpenCliError(
                ...
                help_text="Set VIDEO_CONTENT_OPENCLI to opencli or the built OpenCLI main.js path.",
            ) from error
...
def _write_download_cache(cache_entry: Path, cache_key: str, media_path: Path) -> None:
    ...
    payload = {
        "schema_version": "video-content/download-cache-v1",
        "cache_key": cache_key,
...
def _download_result(
    ...
    *,
    ...
    stat = media_path.stat()
    return {
        "schema_version": "video-content/download-result-v1",
        "provider": "opencli",
        "cache_key": cache_key,
        "cache_hit": cache_hit,

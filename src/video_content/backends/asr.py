...
def resolve_asr_backend(
    ...
    python_executable = _resolve_executable(
        options.python_executable or os.getenv("VIDEO_CONTENT_ASR_PYTHON"),
        fallback=None,
    )
    if python_executable is None:
        raise AsrUnavailable(
            "Qwen3-ASR Python is not configured. Set VIDEO_CONTENT_ASR_PYTHON "
            "or pass --asr-python."
        )
    ffmpeg_executable = _resolve_executable(
        options.ffmpeg_executable or os.getenv("VIDEO_CONTENT_FFMPEG"),
        fallback="ffmpeg",
    )
    if ffmpeg_executable is None:
        raise AsrUnavailable(
            "ffmpeg is required for Qwen3-ASR audio chunking. Set "
            "VIDEO_CONTENT_FFMPEG or pass --ffmpeg."
        )

    model = (options.model or os.getenv("VIDEO_CONTENT_QWEN_ASR_MODEL") or "").strip()
    aligner = (
        options.aligner or os.getenv("VIDEO_CONTENT_QWEN_ALIGNER_MODEL") or ""
    ).strip()
    if not model:
        raise AsrUnavailable(
            "Qwen3-ASR model is not configured. Set "
            "VIDEO_CONTENT_QWEN_ASR_MODEL to a local model directory."
        )
    if not aligner:
        raise AsrUnavailable(
            "Qwen3 Forced Aligner is not configured. Set "
            "VIDEO_CONTENT_QWEN_ALIGNER_MODEL to a local model directory."
        )
